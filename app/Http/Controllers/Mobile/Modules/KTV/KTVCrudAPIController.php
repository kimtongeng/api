...
class KTVCrudAPIController extends Controller
{
     ...
     *
     */
    public function addKTV(Request $request)
    {
        ...
        $request->merge([
            ...
            Business::BUSINESS_TYPE_ID => BusinessTypeEnum::getKtv(),
        ]);
        ...
        if($business->save()) {
            ...
            if (!empty($request->input($business->{Business::IMAGE}))) {
                $image = StringHelper::uploadImage($request->input(Business::IMAGE), ImagePath::ktvLogo);
            ...
            if (!empty($request->input('gallery_photo'))) {
                foreach ($request->input('gallery_photo') as $key => $obj) {
                    $data = [
                        GalleryPhoto::TYPE => GalleryPhotoType::getKtvCover(),
                    ...
                    if ($gallery_photo->save()) {
                        //Upload Image
                        $image = StringHelper::uploadImage($obj['image'], ImagePath::ktvCover);
            ...
            if ($contactOwner->{Contact::IS_KTV_OWNER} == IsBusinessOwner::getNo()) {
                $contactOwner->{Contact::IS_KTV_OWNER} = IsBusinessOwner::getYes();
                $contactOwner->save();
            }
            ...
            $response = [
                ...
                'ktv' => [
                    ...
                    'video_link' => $business->video_link,
     ...
     *
     */
    public function editKTV(Request $request)
    {
        $this->checkValidation($request);
                ...
                if (!empty($request->input('gallery_photo'))) {
                    foreach ($request->input('gallery_photo') as $key => $obj) {
                        ...
                        if (empty($obj[GalleryPhoto::ID])) {
                            ...
                            if ($gallery_photo->save()) {
                                //Upload Cover
                                $image = StringHelper::uploadImage($obj[GalleryPhoto::IMAGE], ImagePath::ktvCover);
                                $gallery_photo->{GalleryPhoto::IMAGE} = $image;
                                $gallery_photo->save();
                            }
                        } else {
                ...
                if (!empty($request->input('deleted_gallery_photo'))) {
                    foreach ($request['deleted_gallery_photo'] as $obj) {
                        if (!empty($obj[GalleryPhoto::ID])) {
                            ...
                            StringHelper::deleteImage($gallery_photo->{GalleryPhoto::IMAGE}, ImagePath::ktvCover);
                        }
                    }
                }
     ...
     *
     */
    public function deleteKTV(Request $request)
    {
        ...
        if ($business->delete()) {
            // Delete Logo
            StringHelper::deleteImage($business->{Business::IMAGE}, ImagePath::ktvLogo);

            // Delete Gallery Photo
            $gallery_photo_list = GalleryPhoto::where(GalleryPhoto::TYPE, GalleryPhotoType::getKtvCover())
            ...
            //Check and set KTV owner if delete
            $ktvByContact = Business::where(Business::CONTACT_ID, $request->input('current_user_id'))
            ->where(Business::BUSINESS_TYPE_ID, BusinessTypeEnum::getKtv())
            ->get();
            if (count($ktvByContact) == 0) {
                //Remove seller owner from contact
                $contact = Contact::find($request->input('current_user_id'));
                $contact->{Contact::IS_KTV_OWNER} = IsBusinessOwner::getNo();
                $contact->save();
            }
        }
     ...
     *
     */
    public function getMyKTV(Request $request)
    {
        $this->validate($request, [
            'filter' => 'required',
